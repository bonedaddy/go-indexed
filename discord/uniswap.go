...
func (c *Client) uniswapPercentChangeHandler(ctx *dgc.Ctx) {
	...
	case "degen10-dai", "degen-dai":
		...
		if err != nil {
			ctx.RespondText("failed to get price")
			c.logger.Error("failed to calculate price change", zap.Error(err), zap.String("asset", "degen10"))
			return
		}
		var changed string
		if (price * 100) < 0 {
	...
	case "nftp", "nftp-dai":
		...
		if err != nil {
			ctx.RespondText("failed to get price")
			c.logger.Error("failed to calculate price change", zap.Error(err), zap.String("asset", "nftp"))
			return
		}
		var changed string
		if (price * 100) < 0 {
