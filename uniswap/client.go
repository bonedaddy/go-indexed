...
func (c *Client) GetReserves(token0, token1 common.Address) (rsrv *Reserve, err error) {
	...
	defer cancel()
	// hacky workaround for https://github.com/ethereum/go-ethereum/issues/22100
	// this allows us to save the panicking goroutine
	defer func() {
		reserves := struct {
			Reserve0           *big.Int
			Reserve1           *big.Int
		...
		if err != nil {
			log.Println("failed to get reserves: ", err)
			return
		}
		...
		rsrv = &Reserve{Reserve0: reserves.Reserve0, Reserve1: reserves.Reserve1, BlockTimestampLast: reserves.BlockTimestampLast}
	}()
	return
}
